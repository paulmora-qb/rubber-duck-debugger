...
def _holdings_section_html(
    ...
    n = len(latest)
    header = (
        f"<h3 style='margin-top:24px'>{_fmt_strategy_name(strategy)} &mdash; "
    ...
    tail_row = (
        ...
        f"<td>&hellip; and {tail_count} others</td>"
    ...
    turnover_row = (
        ...
        f"Avg weight: {latest['weight'].mean() * 100:.2f}% &middot; "
...
<body style="font-family:Arial,sans-serif;max-width:940px;margin:auto;padding:24px">
  <h2 style="border-bottom:2px solid #333;padding-bottom:8px">
    RDD Daily Portfolio Performance
  </h2>
  ...
  <p style="color:#888;font-size:11px;margin-top:32px">
    Generated by RDD &middot; {pd.Timestamp.now().strftime("%Y-%m-%d %H:%M UTC")}
  </p>
</body>
</html>
"""

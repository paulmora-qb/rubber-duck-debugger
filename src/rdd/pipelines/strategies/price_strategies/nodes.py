...
def _rebalance_dates(params: dict[str, Any]) -> list[pd.Timestamp]:
    """Return rebalance dates for the strategy.

    When ``start_date`` is set (backtest mode): every Monday from that date to
    today. When absent (live mode): the last ``backfill_weeks`` Mondays.
    """
    ...
    if start_date:
        ...
        first_monday = first + pd.Timedelta(days=days_ahead)
        return list(pd.date_range(start=first_monday, end=last_monday, freq="W-MON"))
